...
import com.ai.infrastructure.mapper.RagChatMapper;
...
public class RagChatSessionService {
    ...
    public Long prepareStreamMessage(Long sessionId, String question) {
        ...
        userMessage.setMessageOrder(nextOrder);             // 偶数顺序
        userMessage.setCompleted(true);                     // 用户信息输入完成
        ...
        assistantMessage.setContent("");                        // ai消息还是空,用来占位
        assistantMessage.setMessageOrder(nextOrder + 1);        // 奇数顺序
        assistantMessage.setCompleted(false);                   // 未完成状态
        assistantMessage = messageRepository.save(assistantMessage);

        // 更新会话消息数量
        session.setMessageCount(nextOrder + 2);         // 用户和ai各发一条消息
    ...
    public void completeStreamMessage(Long messageId, String content) {
        ...
        message.setContent(content);            // 将流式数据填充到消息中
        message.setCompleted(true);             // 完成状态
