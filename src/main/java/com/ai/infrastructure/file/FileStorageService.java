...
import com.ai.common.config.StorageConfigProperties;
...
public class FileStorageService {
    ...
    private String uploadFile(MultipartFile file, String prefix) {
        ...
        try {
            ...
            log.info("读取上传文件失败: {}", e.getMessage(), e);
            throw new BusinessException(ErrorCode.STORAGE_UPLOAD_FAILED, "文件读取失败");
        } catch (S3Exception e) {
            log.info("上传文件到RustFS失败: {}", e.getMessage(), e);
    ...
    private String convertToPinyin(String input) {
        ...
        // input.toCharArray() 是获取字符串的每个字符的数组
